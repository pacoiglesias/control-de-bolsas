...
title Control Bolsas - Instalar v7.0.2 - Auditoria Externa
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v7.0.2  -  Auditoria completa de v7.0.0
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v7.0.2 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   URGENTE: PORTAL DEL MAQUILADOR CORREGIDO
echo.
echo     - "Error al cargar ordenes" despues de poner el PIN
echo       correcto: la funcion del servidor consultaba por un
echo       campo (isArchived) que probablemente no tenia indice
echo       de Firestore creado todavia, y la consulta fallaba
echo       directo en vez de solo mostrar datos incompletos.
echo       Ya no depende de ese indice.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v7.0.2.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v702_%RANDOM%"
...
set "BACKUP_LOG=%TEMP%\cb_respaldo_log_702.txt"
...
set "MERGE_LOG=%TEMP%\cb_instalacion_log_702.txt"
echo  [..] Instalando la v7.0.2...
echo.
