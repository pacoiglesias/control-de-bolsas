...
title Control Bolsas - Instalar v7.0.1 - Auditoria Externa
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v7.0.1  -  Auditoria completa de v7.0.0
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v7.0.1 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   AUDITORIA COMPLETA DE TU VERSION v7.0.0:
echo.
echo     - 4 flujos (asignar CR rapido, facturar rapido,
echo       cobrar rapido, Recalcular Precios) guardaban las
echo       facturas de forma directa, sin sincronizar el campo
echo       del que dependen TODAS las consultas del sistema --
echo       una factura recien tocada podia desaparecer de
echo       Dashboard/Cobranza hasta el siguiente guardado
echo       completo del expediente. El de Recalcular Precios
echo       era el mas grave: afecta a TODOS los expedientes
echo       abiertos de una sola vez.
echo     - El boton "Recibida del Contador -^> CAJA" (con la
echo       confirmacion de monto real vs esperado) se habia
echo       perdido al separar la factura en su propio
echo       componente -- restaurado completo.
echo     - Mismo bug de exclusion silenciosa de Firestore ya
echo       visto antes (where con "!=") corregido de nuevo en
echo       tu copia de InvoicesContext.tsx.
echo.
echo   Revision detallada de los 30+ archivos de tu version en
echo   AUDIT_NOTEBOOK.md, iteracion 87.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v7.0.1.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v701_%RANDOM%"
...
set "BACKUP_LOG=%TEMP%\cb_respaldo_log_701.txt"
...
set "MERGE_LOG=%TEMP%\cb_instalacion_log_701.txt"
echo  [..] Instalando la v7.0.1...
echo.
...
REM nadie se de cuenta -- exactamente el problema que causaba que las
REM correcciones nunca se vieran reflejadas despues de desplegar.
echo.
