...
title Control Bolsas - Instalar v6.59.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.59.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.59.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   *** URGENTE: BOTON PARA RESTAURAR EXPEDIENTE ELIMINADO ***
echo.
echo     - No existia forma de deshacer "Eliminar Expediente"
echo       desde la interfaz. Ahora, al abrir un expediente ya
echo       eliminado, aparece el boton "Restaurar Expediente"
echo       en vez de "Eliminar" -- revierte el borrado sin
echo       tocar Firebase Console a mano.
echo.
echo   [!] Despues de instalar: abre el expediente de tus 10
echo       contrarecibos (el que se elimino por accidente hoy)
echo       y presiona "Restaurar Expediente".
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.59.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v659_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.59.0 INSTALADA  -  CICLO 11
echo.
