...
title Control Bolsas - Instalar v6.18.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.18.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.18.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   CICLO 30 - Adelanto a Andres visible otra vez:
echo     - La migracion inicial nunca guardo a quien iba el
echo       pago -- por eso el saldo con Andres salia en rojo
echo       por el monto completo del anticipo.
echo     - Nueva herramienta en /seed: "Reparar movimientos
echo       sin proveedor" -- completa el dato faltante sin
echo       tocar montos ni fechas.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.18.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v618_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.18.0 INSTALADA  -  CICLO 11
echo.
