...
title Control Bolsas - Instalar v6.17.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.17.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.17.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   CICLO 29 - Panel, CAJA, catalogo y un boton roto:
echo     - "Notificar al cliente" no tenia destinatario en el
echo       mailto -- corregido, con campo de correo nuevo.
echo     - Panel principal en 3 secciones, mas claro.
echo     - "Total Vendido" indica que es de todo el historial.
echo     - "Caja Chica" renombrado a "CAJA" en toda la interfaz.
echo     - Catalogo ahora editable: alta, edicion y borrado.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.17.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v617_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.17.0 INSTALADA  -  CICLO 11
echo.
