...
title Control Bolsas - Restaurar v6.0.0 reparada
color 0B
cls
echo.
echo  ============================================================
echo    RESTAURAR v6.0.0 REPARADA  -  proyecto completo
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.0.0 ya reparado
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   Restaura tu v6.0.0 (agregacion server-side, paginacion,
echo   deshacer cobros en bloque, retiro de la IA) YA REPARADA:
echo   corregido el hook condicional que reventaba OrderModal al
echo   cambiar de pestana, el importe NaN en la compra a Andres,
echo   y los 4 errores que impedian que compilara.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.0.0-REPARADA.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
echo  [..] Restaurando la v6.0.0 reparada...
echo.
...
echo.
echo  ============================================================
echo    v6.0.0 RESTAURADA Y REPARADA
echo.
...
echo     1  Revisa la reparacion en AUDIT_NOTEBOOK.md y CHANGELOG.md
echo     2  INSTALL_AND_DEPLOY.bat   (build + deploy a Firebase)
