...
title Control Bolsas - Instalar v6.16.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.16.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.16.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   CICLO 28 - Compras con contexto real y proactivo:
echo     - Folio, Cliente y Fecha de Entrega en cada compra
echo     - Tarjeta "Entregas Atrasadas de Andres"
echo     - Buscador por folio/cliente
echo     - Boton "Registrar Entrega" sin salir de Compras,
echo       usando la misma logica compartida con el expediente
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.16.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v616_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.16.0 INSTALADA  -  CICLO 11
echo.
