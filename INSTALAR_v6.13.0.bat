...
title Control Bolsas - Instalar v6.13.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.13.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.13.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   CICLO 25 - "Pendiente de Facturar" reintegrado:
echo     - Se habia perdido al no subirse antes a GitHub.
echo     - Tarjeta en el panel, filtro renombrado, aviso en
echo       el expediente -- ya reconstruido sobre la base
echo       correcta del Ciclo 24 (sin el boton duplicado).
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.13.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v613_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.13.0 INSTALADA  -  CICLO 11
echo.
