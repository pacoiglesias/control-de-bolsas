...
title Control Bolsas - Instalar v6.19.0 - Ciclo 7
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v6.19.0  -  CICLO 11 (carga inicial)
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar el proyecto v6.19.0 completo
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   CICLO 31 - Vencidos por fecha, bitacora al dia:
echo     - "Vencido" dependia solo del job diario de medianoche;
echo       ahora cuenta por fecha en vivo, igual que Cobranza.
echo     - Bitacora de Parches del sistema (dentro de la app)
echo       estaba congelada en v6.8.0 -- completada hasta v6.18.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-v6.19.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v619_%RANDOM%"
...
echo.
echo  ============================================================
echo    v6.19.0 INSTALADA  -  CICLO 11
echo.
