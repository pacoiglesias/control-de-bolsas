...
title Control Bolsas - Instalar Ciclo 4 (v5.9.0)
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR AUDITORIA CICLO 4  -  v5.8.1 -^> v5.9.0
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Lo unico que hace es:
echo     - respaldar tu proyecto completo antes de tocar un archivo
echo     - copiar encima los 21 archivos de este parche
echo     - respetar lo tuyo: .env, .firebaserc, node_modules,
echo       dist, .git y cualquier archivo que no venga en el ZIP
echo.
echo   Incluye: seguridad en impresion (HTML sin escapar), fin de
echo   condiciones de carrera en OrderModal y Cobranza, sonido
echo   unificado con interruptor, rutas con carga diferida
echo   (bundle principal 582kB -^> 36.7kB), y limpieza de Functions.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "control-bolsas-ciclo4-v5.9.0.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_ciclo4_%RANDOM%"
...
echo     1  Revisa el ciclo 4 en AUDIT_NOTEBOOK.md y CHANGELOG.md
