...
<body style="margin: 0; background: #ffffff;">
  <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; padding: 36px; color: #0f172a; max-width: 800px; margin: 0 auto;">
      ...
      <tbody>
        ${(statement.ledger || []).map((r: any) => `
          <tr style="border-bottom: 1px solid #e2e8f0;">
            ...
            <td style="padding: 8px 10px; font-weight: 600;">${String(r.concept || '').replace(/[\u{1F300}-\u{1F9FF}]/gu, '')}</td>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Comprobante de Entrega · OC ${h.folio || ''}</title>
  <style>
    @media print {
      body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      @page { margin: 12mm; }
...
<body style="margin: 0; background: #ffffff;">
  <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; padding: 28px; color: #0f172a; max-width: 600px; margin: 0 auto; border: 2px solid #7c3aed; border-radius: 12px; background: #ffffff;">
    ...
    <div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin-bottom: 18px; border: 1px solid #e2e8f0;">
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 13px;">
        ...
        <div>
          ...
          <div style="font-weight: 900; color: #059669; font-size: 22px; margin-top: 2px;">${(h.kilos || 0).toLocaleString('es-MX')} kg</div>
        </div>
