...
El script hace todo: valida el CLI, renueva la sesión si venció, selecciona el proyecto, guarda la clave
...
| `CONECTAR_FIREBASE.bat` | Renueva la sesión del CLI. Corre esto cuando veas "Authentication Error". |

### Errores frecuentes y qué significan

| Mensaje | Qué es realmente |
|---|---|
| `Authentication Error: Your credentials are no longer valid` | La sesión del Firebase CLI caducó. **No** tiene que ver con la clave de Gemini. → `CONECTAR_FIREBASE.bat` |
| `Assertion failed: resolving hosting target of a site with no site name` | Consecuencia del anterior: sin sesión válida el CLI no puede resolver el proyecto y falla al llegar a Hosting. Se arregla solo al reconectar. |
| `npm warn EBADENGINE ... required: node 20, current: v24` | Solo un aviso. Tu Node local es más nuevo que el runtime de las funciones; no impide nada. El runtime desplegado se fija en `functions/package.json`. |
| `Your project must be on the Blaze plan` | Functions 2ª gen y Secret Manager exigen plan de pago. |
| `Secret GOOGLE_GENAI_API_KEY does not exist` | → `CONFIGURAR_CLAVE_GEMINI.bat` |

### Sobre las vulnerabilidades de `npm audit`

Vienen de dependencias transitivas de `firebase-functions` y `genkit`, casi todas en
herramientas de compilación que nunca se ejecutan en producción. **No corras
`npm audit fix --force`**: sube versiones mayores sin avisar y rompe el despliegue. Cuando
Google publique versiones parcheadas, se actualizan con `npm update`.
