...
title Control Bolsas - Instalar v8.8.6 - Revision de Lista Externa
color 0B
cls
echo.
echo  ============================================================
echo    INSTALAR v8.8.6  -  Revision de Lista Externa de Hallazgos
echo  ============================================================
echo.
echo   Este instalador NO BORRA NADA. Solo copia 9 archivos:
echo     - src/components/Cobranza/InvoiceDrawer.tsx
echo     - src/components/OrderModal/InvoiceWidget.tsx
echo     - src/components/OrderModal/useInvoiceActions.ts
echo     - src/lib/finance.ts
echo     - src/lib/bridge.ts
echo     - eslint.config.js
echo     - package.json / functions/package.json / CHANGELOG.md
echo   Respalda tu proyecto completo antes de tocar nada. Respeta
echo   .env, .firebaserc, node_modules, dist y .git.
echo.
echo   QUE CORRIGE:
echo     - Bug real: "Recibir en Caja" desde el widget de factura
echo       pasaba un config vacio -- podia fallar con facturas viejas
echo       sin datos financieros completos. Encontrado al quitar
echo       varios tipos "any" (dynamicConfig) que lo escondian.
echo     - Limpieza de condiciones redundantes en inferDepartment().
echo     - ESLint ahora reconoce globals de Node en Cloud Functions.
echo     - Numero magico en bridge.ts ahora es una constante.
echo     - Se revisaron 3 hallazgos mas de la lista y NO eran reales
echo       (detalles en el CHANGELOG) -- no se toco nada por ellos.
echo.
...
if !CUANTOS!==0 (
  ...
  echo      Deja "PARA-CLAUDE_parche_v8_8_6.zip" en esta misma carpeta.
  pause & exit /b 1
)
if !CUANTOS! GTR 1 (
  echo.
...
set "TMPDIR=%TEMP%\cb_v886_%RANDOM%"
...
set "BACKUP_LOG=%TEMP%\cb_respaldo_log_886.txt"
...
REM ---------- 6. Copiar SOLO los archivos de este parche ----------
...
set "MERGE_LOG=%TEMP%\cb_instalacion_log_886.txt"
echo  [..] Instalando la v8.8.6...
echo.
...
echo     1  INSTALAR_BUILD_DEPLOY.bat   (instala deps + build + deploy)
echo     2  PUSH_TO_GIT.bat             (subir a GitHub + respaldo)
echo.
set /p SIG="  Corro INSTALAR_BUILD_DEPLOY.bat ahora? (s/n): "
if /i "!SIG!"=="s" (
  if exist "INSTALAR_BUILD_DEPLOY.bat" (
    call INSTALAR_BUILD_DEPLOY.bat
  ) else (
    echo  [!] No encontre INSTALAR_BUILD_DEPLOY.bat en el proyecto.
  )
)
exit /b 0
